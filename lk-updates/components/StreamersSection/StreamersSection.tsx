...
const PLACEHOLDER_STREAMERS: Streamer[] = Array.from({ length: 6 }, (_, i) => ({
  id: i + 1,
  name: '~△b9lBoJl.HoCuT.DEMIX~',
  description: 'Играет в различные компьютерные игры, вайбкодит, а так же не пьет уже много месяцев',
  imageUrl: 'https://www.figma.com/api/mcp/asset/c28cdebf-e396-4128-837b-f5a160f4c84d',
...
export default function StreamersSection({ streamers }: StreamersSectionProps) {
  const items = streamers && streamers.length > 0 ? streamers : PLACEHOLDER_STREAMERS;
        ...
        {items.map((streamer, i) => (
          ...
          >
            <div className={styles.cardImage}>
              <img src={streamer.imageUrl} alt={streamer.name} className={styles.cardImg} />
            </div>
