        ...
        <section className={styles.contentGrid}>
          <div className={styles.mainColumn}>
            ...
            <div className={styles.tabPanel}>
              ...
              {tab === 'activity' && <div className={styles.sectionStack}>{activity?.collector_state && activity.collector_state !== 'active' && activity.collector_message ? <div className={`${styles.statusNotice} ${activity.collector_state === 'bot_unavailable' ? styles.statusNoticeWarning : ''}`}><strong>{activity.collector_state === 'bot_unavailable' ? '?????????? Discord ??????????.' : '?????????? ???? ?????.'}</strong><span>{activity.collector_message}</span>{activity.last_presence_sync_at && <span>????????? ????????????? presence: {formatDate(activity.last_presence_sync_at, 'datetime')}</span>}</div> : null}<div className={styles.activityOverview}><div className={styles.metricCard}><span>?????????</span><strong>{activity?.message_count ?? 0}</strong></div><div className={styles.metricCard}><span>????????? ?????</span><strong>{activity?.voice_hours ?? 0}</strong></div></div><div className={styles.panelCard}><div className={styles.panelHeader}><h3>????????? ?????????</h3></div>{activity?.recent_messages?.length ? <div className={styles.sectionStack}>{activity.recent_messages.map((item, index) => <article key={`${item.created_at ?? index}-${item.channel ?? 'message'}`} className={styles.listCard}><div className={styles.listCardTitle}>{item.channel || '??? ??????'}</div><div className={styles.listCardMeta}>{item.type && <span>???: {item.type}</span>}{formatDate(item.created_at, 'datetime') && <span>{formatDate(item.created_at, 'datetime')}</span>}</div></article>)}</div> : <p className={styles.emptyText}>??? ?????? ?? ??????????.</p>}</div><div className={styles.panelCard}><div className={styles.panelHeader}><h3>????????? ????????? ??????</h3></div>{activity?.recent_voice?.length ? <div className={styles.sectionStack}>{activity.recent_voice.map((item, index) => <article key={`${item.joined_at ?? index}-${item.channel ?? 'voice'}`} className={styles.listCard}><div className={styles.listCardTitle}>{item.channel || '??? ??????'}</div><div className={styles.listCardMeta}>{formatDate(item.joined_at, 'datetime') && <span>??????: {formatDate(item.joined_at, 'datetime')}</span>}{formatDate(item.left_at, 'datetime') && <span>?????: {formatDate(item.left_at, 'datetime')}</span>}{typeof item.duration_minutes === 'number' && <span>????????????: {item.duration_minutes} ???.</span>}</div></article>)}</div> : <p className={styles.emptyText}>??? ?????? ?? ????????? ??????????.</p>}</div></div>}
          ...
          <aside className={styles.sidebar}>
            <div className={styles.sideCard}><div className={styles.sideCardHeader}><span className={styles.sideCardTitle}><Award size={14} />??????? ??????????</span>{isOwnProfile && <button className={styles.sideActionBtn} onClick={() => { void loadOnce('achievements', () => axios.get<Achievement[]>(`${API_URL}/api/achievements/user/${encodedProfileIdentifier}?completed_only=true`).then((r) => r.data), setAchievements); setAchievementsOpen(true) }} title="??????? ?????????? ??? ???????" aria-label="??????? ?????????? ??? ???????"><Edit2 size={14} /></button>}</div>{showcaseItems.length === 0 ? <p className={styles.sideEmpty}>????????? ?????????? ???? ???.</p> : <div className={styles.showcaseGrid}>{showcaseItems.map((item) => <div key={item.id} className={styles.showcaseBadge} title={item.name}><span className={styles.showcaseIcon}>{item.icon}</span><span className={styles.showcaseName}>{item.name}</span></div>)}</div>}</div>
